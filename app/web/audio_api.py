...
from typing import List, Optional, Any, Iterable, Iterator
import httpx
...
def _require_manage_docs(user: UserInDB) -> None:
    #todo _require_manage_docs = “强制要求用户拥有 kb.manage_docs 权限，否则立刻拒绝”
    # 它不返回值，只做一件事：
    # 👉 不满足就抛异常
...
def _normalize_visibility(v: str) -> str:
    #todo 把用户输入的 visibility 做“安全归一化”，只允许白名单值，其余全部降级为 public。
    v = (v or "").strip().lower()
...
#todo 统一计算「用户可访问的音频可见性集合」，并在需要时对某个具体文档做权限校验。
#todo 1 算出“这个用户能看到哪些 visibility”
...
def _get_allowed_set(user: UserInDB) -> set[str]:
    #todo _get_allowed_set = 为“热路径权限判断”准备的set版本可见性集合热路径（HotPath）：👉 在系统运行中被“非常频繁执行”的代码路径
    # todo _get_allowed_set 执行流程
    #   │
...
def _compute_allowed_visibilities(user: UserInDB) -> List[str]:
    #todo 计算“当前用户最终允许访问的音频可见性列表”并强制保证 public 一定在其中（且排在最前）。
    # todo _compute_allowed_visibilities 执行流程
    #   │
...
def _ensure_can_access_visibility(user: UserInDB, doc_visibility: str) -> List[str]:
    #todo _ensure_can_access_visibility = 对单个音频文档的可见性做强制访问校验（硬闸）
    # todo _ensure_can_access_visibility 执行流程
    #   │
...
def _absolute_base(request: Request) -> str:
    #todo _absolute_base = 生成当前请求上下文下的绝对 URL 前缀
    # todo _absolute_base 执行流程
    #   │
...
def _clip_url(base: str, audio_id: str, start_ms: int, end_ms: int) -> str:
    #todo _clip_url = 将音频片段定位信息转换为可回放的 clip HTTP URL
    # todo _clip_url 执行流程
    #   │
    #   ├─ 接收 base（请求绝对地址前缀）
...
def _build_langchain_messages(messages: list[dict[str, str]]):
    #todo _build_langchain_messages = 业务 message → LangChain SDK 消息适配层
    # todo _build_langchain_messages 执行流程
    #   │
    #   ├─ 遍历自定义 messages 列表
    #   ├─ 读取 role / content 字段
    #   ├─ role == "system"
...
def _openai_chat_complete(*, messages: list[dict[str, str]]) -> str:
    #todo _openai_chat_complete = 同步 LLM 调用 → 返回最终文本答案
    # todo _openai_chat_complete 执行流程
    #   │
    #   ├─ 获取 LLM 实例（get_llm）
...
def _openai_stream(*, messages: list[dict[str, str]]) -> Iterable[str]:
    #todo _openai_stream = LLM → token/chunk 流 → Python generator
    # todo _openai_stream 执行流程
    #   │
    #   ├─ 获取 LLM 实例（get_llm）
    #   ├─ 将 messages 转换为 LangChain 消息格式
...
def _build_rag_messages(question: str, citations: list[AudioCitation], system_prompt: Optional[str]) -> list[
    dict[str, str]]:
    #todo _build_rag_messages = 把「音频检索结果」转成「受约束的 LLM 输入上下文」
    # todo _build_rag_messages 执行流程
    #   │
    #   ├─ 确定 system prompt
...
def _search_audio_segments(vs, query: str, allowed_vis: list[str], k: int, audio_id: Optional[str] = None):
    #todo _search_audio_segments = 带权限过滤的“高召回”音频向量检索函数
    # todo _search_audio_segments 执行流程
    #   │
...
def _build_audio_hits(docs_scores, allowed_vis_set: set[str], base: str, mode: str = "hit"):
    ...
    #todo 把「向量检索结果（LangChain doc + score）」
    # 👉 过滤 + 去重 + 权限校验
    # 👉 转成「业务层可直接返回的 AudioSearchHit / AudioCitation」
    # hits 是「音频向量检索后的 最终可返回结果列表」——已经 去重 + 权限校验 + 业务封装 过的音频片段集合。
    results, seen = [], set()  #todo seen用于防止重复片段，比如多个检索结果指向相同音频区间     初始化：结果集 + 去重容器⚠️ 向量检索非常容易出现同一片段被多次命中所以要用seen
    for doc, score in docs_scores:#遍历每个向量结果
        ...
        if not (audio_id and segment_id):#如果缺字段，直接丢弃：👉 这是容错 + 防脏数据
            continue
        try:
            start_ms, end_ms = int(md.get("start_ms", 0)), int(md.get("end_ms", 0))#todo 校验时间区间合法性  这是非常重要的一层 安全网：
                                                                                    # 防止 ES / 向量库脏数据
                                                                                    # 防止 clip 接口被打爆
        except Exception:
            continue
        ...
        key = (audio_id, segment_id, start_ms, end_ms)#todo 构造“业务级唯一键   ”去重“音频 + 段 + 时间区间” = 真实世界唯一片段
        if key in seen:
            continue
        seen.add(key)  # 生成唯一key，即同一个片段唯一标识，所以这里用了set集合
        db_doc = audio_db.get_audio_document(audio_id)#todo 查询 audio_document（上层事实源）
        if not db_doc:
            continue
        if (db_doc.get("visibility") or "").strip().lower() not in allowed_vis_set:#todo 权限过滤（最终兜底）
            continue
        text = (doc.page_content or "").strip()#todo 拿正文文本
        #todo 根据 mode 构造不同 DTO
        # 搜索模式（query / search）
        if mode == "hit":  #todo hit搜索结果列表/query，返回AudioSearchHit
...
async def ingest_audio(
    ...
    if audio_db.is_audio_running(audio_id):#todo 防止同一音频并发处理🚫 这是你避免状态炸裂的第一道防线
        raise HTTPException(status_code=409, detail="audio is running, try later")

    existed = audio_db.get_audio_document(audio_id)
    if existed and not overwrite:#todo 默认不允许覆盖  overwrite=true → 允许，但：记录 old_stored_path交给 job 决定是否删除
    ...
    raw_bytes = await file.read()#todo 保存原始文件
    if not raw_bytes:
        raise HTTPException(status_code=400, detail="Empty file")
    raw_path.write_bytes(raw_bytes)

    audio_db.upsert_audio_document(#todo 👉 这是“事实源”，worker 以后只更新它
        audio_id=audio_id,
    ...
    audio_job_db.create_job(#todo 创建 audio_job
        job_id,
        audio_id,
    ...
    async_result = audio_ingest_task.apply_async(#todo Celery 投递   不关心执行结果只拿 task_id
        args=[job_id, audio_id],
        queue=getattr(settings, "celery_audio_queue", "audio"),
    )
    audio_job_db.bind_task(job_id, async_result.id)#todo 绑定 task_id
...
#todo GET /audio/jobs/{job_id} = 查询单个音频作业状态的管理接口
# 👉 只给 kb.manage_docs
...
#todo POST /jobs/{job_id}/cancel = 音频处理任务的“管理态取消接口”
# 👉 只能管理员用
# 👉 只是发出取消请求，不是强杀
...
#todo GET /docs/{audio_id} = 管理态接口（admin / ops）
# 👉 只给「能管理文档的人」看
...
#todo query_audio = 纯检索接口 👉 只做「向量搜索 + 权限过滤 + 命中片段返回」
def query_audio(#todo 拿到用户输入的搜索词 q，控制返回数量 k，已认证的 current_user
        request: Request,
    ...
    vs = get_audio_vs()                                                     #todo 拿向量库实例ES / Chroma / Milvus / FAISS 的封装后面所有检索都走它
    base = _absolute_base(request)                                          #todo 构造 base URL（给 clip 用）

    docs_scores = _search_audio_segments(vs, q, allowed_vis, k)             #todo 向量搜索音频片段这是整条链路的“心脏”：
                                                                            # 输入：
                                                                            # 查询文本 q
                                                                            # 可见性 allowed_vis
                                                                            # 返回数量 k
    ...
    hits = _build_audio_hits(docs_scores, allowed_vis_set, base, mode="hit")#todo 命中构建（去重 + DB 校验 + URL）
...
def ask_audio(req: AudioAskReq, request: Request, current_user: UserInDB = Depends(get_current_user)) -> AudioAskResp:
    #todo ask_audio =在用户权限范围内 → 用问题做向量检索 → 找出最相关的音频片段 →
    ...
    try:
        docs_scores = vs.similarity_search_with_score(question, k=fetch_k, filter=where)#todo 向量检索（核心之一）把 question → embedding
    ...
    for doc, score in docs_scores:
        ...
        db_doc = audio_db.get_audio_document(audio_id)#todo 再次查 DB + 再次做权限兜底（非常关键）
        ...
        citations.append(                                                       #todo 构造 citation（ask 的第二个核心）ask 不返回音频，只返回“证据链”
            AudioCitation(  #todo 每个 citation 都是：
                            # LLM 用来回答的证据
                            # 前端用来播放的入口
                audio_id=audio_id,
...
#todo 音频切片 (/audio/docs/{audio_id}/clip)
@router.get("/docs/{audio_id}/clip")
#todo 从一个音频里，按时间范围或按 segment，裁剪一小段 mp3，返回给客户端，并在返回后自动删除临时文件。这是一个 “即时生成 + 临时文件 + 流式返回” 的接口
# 这是一个：
# - 即时生成（ffmpeg 同步）
# - 临时文件（mp3 落盘）
# - FileResponse 流式返回
...
def get_audio_clip(
    ...
    background_tasks.add_task(lambda p=str(clip_path): Path(p).unlink(missing_ok=True))     #todo FastAPI 的 BackgroundTasks 行为是：响应返回给客户端之后，立即执行后台任务
...
def delete_audio_doc_api(audio_id: str, current_user: UserInDB = Depends(get_current_user)):#todo在执行函数之前，先调用 get_current_user()，把返回值作为 current_user 传进来
    _require_manage_docs(current_user)                                                      # UserInDB = “从数据库里读出来的、可信的用户对象”
    ...
    doc = audio_db.get_audio_document(audio_id)                                             #todo 如果数据库里根本没有这个 audio，就直接返回 404，后面的逻辑全部不应该再执行
    if not doc:
        raise HTTPException(status_code=404, detail="audio not found")

    # 1) delete vectors
    vec_deleted = 0                                                   #todo先初始化为 0
    try:
        vec_deleted = int(delete_by_audio_id(audio_id) or 0)          #todo尝试删除向量
    except Exception:
        vec_deleted = 0

    # 2) delete db rows
    db_deleted = audio_db.delete_audio_document_cascade(audio_id)     #todo 在数据库中，按 audio_id 执行一次「级联删除」级联删除：删除“这个 audio 相关的所有数据库数据”

    # 3) delete files
    files_deleted: list[str] = []                                     #todo 记录“成功删除了哪些文件”
    errors: list[str] = []                                            #todo 记录“删除过程中发生了哪些非致命错误”

    # raw audio
    #todo 从 DB 里取 stored_path
    # 构造文件路径
    # 存在且是文件才删
    # 成功 → 记入 files_deleted
    # 失败 → 记入 errors
...
#todo这是一个“触发音频重建索引的后台任务”的 API——校验权限 → 校验状态 → 创建 Job → 派发 Celery Task → 返回任务信息
def reindex_audio_doc_api(audio_id: str, current_user: UserInDB = Depends(get_current_user)):               #todo current_user当前登录用户（FastAPI 依赖注入）
    _require_manage_docs(current_user)

    if audio_db.is_audio_running(audio_id):                                                                 #todo 检查这个 audio 是否已经被某个任务占用
        raise HTTPException(status_code=409, detail="audio is running, try later")
    ...
    #todo 在数据库里创建一个“业务 Job（作业）记录”，用来追踪一次音频重建索引的全过程
    job_id = f"job-{uuid.uuid4().hex[:12]}"
    audio_job_db.create_job(job_id, audio_id, overwrite=False, delete_old_file=False, old_stored_path=None)#todo 这一步 不是“执行任务”，而是：在数据库里登记：我要开始做一件事了

    async_result = audio_reindex_task.apply_async(                                                         #todo 把“业务 job”交给 Celery，生成一个真正会跑的 task
        args=[job_id, audio_id],
        queue=getattr(settings, "celery_audio_queue", "audio"),
    )
    audio_job_db.bind_task(job_id, async_result.id)                                                        #todo 把 task_id 回写到 job 里，完成“绑定”
                                                                                                           #todo job 是“合同”，task 是“施工队”bind_task 是“合同上写明施工队编
    return {                                                                                               #todo 这是后端对前端的正式承诺：“这个音频的 reindex 已经受理了，你可以用这些信息持续跟踪它。”
...
# =========================
#todo 先检索出“允许当前用户看的音频片段（citations）” →
# 再把问题 + citations 丢给 DeepSeek →
# 把模型一边生成的内容，一边通过 SSE 实时推给前端



def _sse(event: str, data: Any) -> bytes:                   #todo _sse 把一个 Python 对象，包装成符合 SSE 协议规范的“事件帧”，再编码成字节，交给 StreamingResponse 发给客户端。
...
#todo 这段代码的作用本质上就是在“区分 / 校验请求体是不是一个合法的 JSON 对象（dict）”，但要说清楚一点，它不是在区分“是不是 JSON 文件”，而是在区分：👉 请求体解析出来之后，是不是一个 JSON object（dict）
    req: dict,
    request: Request,
    current_user: UserInDB = Depends(get_current_user),
):
    ...
    def event_stream():
#todo 这是一个 SSE（Server-Sent Events）事件生成器
